...
if page == "📊 Dashboard Executive":
    ...
    try:
        ...
        std_cycles = cycles_par_moteur.std()
        
        # Filter out useless sensors (from notebook - S1, S5, S6, S10, S16, S18, S19 have variance ≈ 0)
        useful_sensors = ['S2', 'S3', 'S4', 'S7', 'S8', 'S9', 'S11', 'S12', 'S13', 'S14', 'S15', 'S17', 'S20', 'S21']
        
        scaler = StandardScaler()
        data_scaled = scaler.fit_transform(data_train[useful_sensors])
        ...
        # Financial KPIs (from notebook section 9 - more realistic estimates)
        cost_corrective = 50_000      # Maintenance corrective
        cost_preventive = 15_000      # Maintenance préventive
        cost_downtime = 10_000        # Coût d'arrêt par jour
        
        last_rows = data_train.sort_values('cycles').groupby('unit_id').tail(1)
        moteurs_a_risque = (last_rows['rul_true'] <= 30).sum()
        moteurs_critiques = (last_rows['rul_true'] <= 10).sum()
        
        cost_without_model = cost_corrective * FLEET_SIZE
        cost_with_model = cost_preventive * FLEET_SIZE
        cost_avoided = cost_downtime * 0.5 * moteurs_a_risque
        savings = int(cost_without_model - cost_with_model - cost_avoided)
        roi_pct = (savings / cost_with_model) * 100 if cost_with_model > 0 else 0
        ...
        col3.metric("⏱️ Cycles (Avg±Std)", f"{mean_cycles:.0f}±{std_cycles:.0f}")
        ...
        st.markdown("---")
        st.markdown("### 🔗 Analyse de Corrélation des Capteurs Clés")
        st.info("💡 D'après l'analyse du notebook: S7 (Vitesse Fan) et S12 (Débit Carburant) sont fortement corrélés (+0.8127). Cette synchronisation directe est une signature importante de santé du moteur.")
        
        # Create correlation heatmap for useful sensors
        corr_matrix = data_train[useful_sensors[:10]].corr()  # Top 10 for clarity
        ...
        fig_corr.update_layout(height=450, title="Corrélation: Top 10 Capteurs Utiles (Scénario FD001)", margin=dict(t=80, b=50))
        ...
        if len(snap) > 0:
            # risk on snapshot
            snap_scaled = scaler.fit_transform(snap[useful_sensors])
        ...
        st.markdown("---")
        st.markdown("### KPIs financiers (Estimation Réaliste)")
        st.caption("Basé sur l'analyse du notebook - Coûts maintenance corrective vs préventive")
        # Display cost breakdown
        c1, c2, c3, c4 = st.columns(4)
        c1.metric("💸 Coût Corrective (Panne)", f"€{cost_corrective:,}".replace(","," "))
        c2.metric("💚 Coût Préventif", f"€{cost_preventive:,}".replace(","," "))
        c3.metric("⏰ Économies par Moteur", f"€{cost_corrective - cost_preventive:,}".replace(","," "))
        c4.metric("📊 Moteurs < 30 RUL", int(moteurs_a_risque))
        
        f1, f2, f3 = st.columns(3)
        f1.metric("Coût évité (estimé)", f"€ {savings:,}".replace(",", " "))
        f2.metric("ROI maintenance préventive", f"{roi_pct:.0f}%")
        f3.metric("Moteurs critiques (RUL≤10)", int(moteurs_critiques))

        st.caption("Hypothèses réalistes (notebook section 9): maintenance corrective=50k€, préventive=15k€, arrêt/jour=10k€")
...
if page == "🏠 Accueil":
    st.markdown("### 📌 Insights Clés du Notebook (EDA Complète)")
    
    with st.expander("🔍 Voir les résultats de l'analyse notebook"):
        col_insight1, col_insight2 = st.columns(2)
        
        with col_insight1:
            st.markdown("#### 📊 **Section 2-3: Structure des Données**")
            st.write("""
            - **Dataset**: ~100 moteurs, 21 capteurs, données "run-to-failure"
            - **Capteurs utiles**: 14/21 (7 supprimés pour variance quasi-nulle)
            - **Distribution cycles**: Min=50, Moyenne=155, Max=362 (asymétrique droite)
            - **Impact**: Réduction bruit, meilleure prédiction RUL
            """)
            
            st.markdown("#### 🔗 **Section 3.2: Corrélations Critiques**")
            st.write("""
            - **S7 ↔ S12**: r=+0.8127 (synchronisation thermodynamique)
            - **S11 ↔ S12**: r=-0.8469 (déséquilibre opérationnel)
            - **S9**: Quasi-isolé (r≈0.3) → signal complément orthogonal
            - **Action**: Surveiller découplage S7-S12 comme indicateur de dégradation
            """)
        
        with col_insight2:
            st.markdown("#### 🚨 **Section 4: Détection d'Anomalies**")
            st.write("""
            - **Z-Score (σ>3)**: Détecte ~4.3% pics isolés
            - **Isolation Forest**: Détecte ~1,032 combinaisons anormales
            - **Consensus**: Score composite confirme anomalies critiques
            - **Recommandation**: Déclencher alerte si 2 méthodes s'accordent
            """)
            
            st.markdown("#### 🧠 **Section 7-9: Modélisation & KPIs**")
            st.write("""
            - **Meilleur modèle**: RandomForest/GradientBoosting (voir comparaison)
            - **Feature Engineering**: Moyennes/écart-types glissants sur 5/10/20 cycles
            - **KPIs Financiers**:
              - Coût corrective: 50k€
              - Coût préventif: 15k€
              - ROI maintenance préventive: ~230%
            """)
    ...
    try:
        ...
        # Use useful sensors from notebook analysis (S1, S5, S6, S10, S16, S18, S19 have variance ~0)
        useful_sensors = ['S2', 'S3', 'S4', 'S7', 'S8', 'S9', 'S11', 'S12', 'S13', 'S14', 'S15', 'S17', 'S20', 'S21']
        ...
        with col2:
            st.markdown("### 🌡️ Profil Capteurs/Risque (Utiles seulement)")
            st.caption("14/21 capteurs utilisés (7 supprimés pour variance quasi-nulle)")
            data_train['risk'] = risk_levels
            heatmap_data = data_train.groupby('risk')[useful_sensors].mean()
        ...
        # ROW 2: CORRELATION + ANOMALIES (from notebook section 3.2)
        col1, col2 = st.columns(2)
        with col1:
            st.markdown("### 🔗 Corrélation Top Capteurs (Notebook Analysis)")
            st.caption("Top 10 capteurs utiles - Notebook section 3.2")
            corr_matrix = data_train[useful_sensors[:10]].corr()
            fig_corr = go.Figure(data=go.Heatmap(
                z=corr_matrix.values, 
                x=corr_matrix.columns, 
                y=corr_matrix.columns, 
        ...
        with col2:
            st.markdown("### 🔍 Détection Anomalies (Notebook Section 4)")
            
            # Z-Score method (notebook: section 4.1)
            from scipy import stats
            z_scores_all = np.abs(stats.zscore(data_train[useful_sensors], nan_policy='omit'))
            anomalies_zscore = (z_scores_all > 3).sum(axis=1) > 0
            zscore_pct = 100 * anomalies_zscore.sum() / len(data_train)
            
            # Isolation Forest (notebook: section 4.2)
            iso_forest = IsolationForest(contamination=0.05, random_state=42)
            anomalies_if = iso_forest.fit_predict(data_train[useful_sensors])
            if_pct = 100 * (anomalies_if == -1).sum() / len(anomalies_if)
            
            # Composite score (notebook: section 4.3)
            composite_anomalies = (anomalies_zscore.astype(int) + (anomalies_if == -1).astype(int)) / 2
            
            col2a, col2b, col2c = st.columns(3)
            col2a.metric("Z-Score (σ>3)", f"{anomalies_zscore.sum()}", f"{zscore_pct:.1f}%")
            col2b.metric("Isolation Forest", f"{(anomalies_if == -1).sum()}", f"{if_pct:.1f}%")
            col2c.metric("Consensus", f"{(composite_anomalies > 0).sum()}", f"{100*composite_anomalies.mean():.1f}%")
            
            st.caption("📌 Deux méthodes: Z-Score détecte pics isolés, Isolation Forest détecte combinaisons anormales multi-capteurs")
        
        st.markdown("---")
        
        # ROW 3: ALERTS
        st.markdown("### 📋 Alertes & Status Moteurs")
        ...
        st.info(f"⚠️ Total {len(alerts_recent)} observations critiques/alerte dans les {len(data_train)} points de données")
    ...
    try:
        ...
        col1, col2 = st.columns(2)
        with col1:
            st.markdown("### 🎯 Prédiction RUL Interactive")
            st.info("💡 Notebook Section 7: Modélisation avec RandomForest, Gradient Boosting et XGBoost")
            ...
            status = "🔴 CRITIQUE" if rul_current <= RUL_THRESHOLD_CRITICAL else "🟡 ALERTE" if rul_current <= RUL_THRESHOLD_WARNING else "🟢 NORMAL"
            s1, s2, s3 = st.columns(3)
            s1.metric("RUL Actuel", f"{rul_current:.1f} cycles")
            s2.metric("Confiance (IC95%)", f"±{ci:.1f} cycles")
            s3.metric("Statut", status)
        
        with col2:
            st.markdown("### 🧩 Feature Engineering (Notebook Section 5)")
            st.caption("Création de variables glissantes pour séries temporelles")
            useful_sensors = ['S2', 'S3', 'S4', 'S7', 'S8', 'S9', 'S11', 'S12', 'S13', 'S14', 'S15', 'S17', 'S20', 'S21']
            sensor_choice = st.selectbox("Capteur:", useful_sensors, index=4)  # Default to S7
            window = st.slider("Fenêtre glissante:", 3, 60, 15, step=1, help="Nombre de cycles pour moyenne/std glissante")
            df_fe = df_motor.copy()
            df_fe[f'{sensor_choice}_roll'] = df_fe[sensor_choice].rolling(window=window, min_periods=1).mean()
            df_fe[f'{sensor_choice}_std'] = df_fe[sensor_choice].rolling(window=window, min_periods=1).std().fillna(0)
            ...
            fig_fe.add_trace(go.Scatter(x=df_fe['cycles'], y=df_fe[f'{sensor_choice}_roll'], mode='lines', name=f"{sensor_choice} (moyenne lissée)",
                                       line=dict(color=COLOR_PALETTE['secondary'], width=3)))
            fig_fe.update_layout(height=400, xaxis_title="Cycles", yaxis_title="Valeur", margin=dict(t=30, b=50), hovermode='x unified')
            st.plotly_chart(fig_fe, use_container_width=True)
            
            st.caption("💡 Le lissage réduit le bruit et facilite la détection de dégradation progressive (voir Notebook Section 5.2: Mutual Information)")
    ...
    try:
        ...
        st.markdown("### 📊 Analyse Exploratoire - Distribution des Données")
        
        col1, col2 = st.columns(2)
        with col1:
            st.markdown("### 🔎 Variabilité Capteurs Utiles")
            useful_sensors = ['S2', 'S3', 'S4', 'S7', 'S8', 'S9', 'S11', 'S12', 'S13', 'S14', 'S15', 'S17', 'S20', 'S21']
            sensor_stats = data_train[useful_sensors].std().sort_values(ascending=False)
            fig_var = px.bar(
                x=sensor_stats.index,
                y=sensor_stats.values,
                title="Écart-type des 14 Capteurs Utiles",
                labels={'x': 'Capteur', 'y': 'Écart-type'},
                color_discrete_sequence=[COLOR_PALETTE['primary']]
            )
            fig_var.update_layout(height=350, showlegend=False, margin=dict(t=50, b=50))
            st.plotly_chart(fig_var, use_container_width=True)
        
        with col2:
            st.markdown("### 📈 Distribution Cycles par Moteur")
            st.caption("Notebook section 3.1: Distribution asymétrique à droite")
            cycles_per_unit = data_train.groupby('unit_id')['cycles'].max()
            fig_hist = px.histogram(
                cycles_per_unit, 
                nbins=30,
                title="Cycles de Fonctionnement",
                labels={'value': 'Cycles', 'count': 'Nombre de moteurs'},
                color_discrete_sequence=[COLOR_PALETTE['primary']]
            )
            fig_hist.update_layout(height=350, showlegend=False, margin=dict(t=50, b=50))
